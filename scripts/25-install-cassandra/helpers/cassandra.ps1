# --------------------------------------------------------------------------
#  Helper -- Apache Cassandra installer
# --------------------------------------------------------------------------
...
function Install-Cassandra {
    <#
    .SYNOPSIS
        Installs Apache Cassandra and verifies the installation.
    ...
    if ($cmd) {
        ...
        Save-ResolvedData -ScriptFolder "25-install-cassandra" -Data @{
    ...
    if ($hasCustomPath) {
        $dbInstallDir = Join-Path $InstallPath "cassandra"
