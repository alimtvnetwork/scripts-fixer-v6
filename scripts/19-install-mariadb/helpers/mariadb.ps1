...
function Install-Mariadb {
    <#
    .SYNOPSIS
        Installs MariaDB and verifies the installation.
    ...
    if ($cmd) {
        ...
        Save-ResolvedData -ScriptFolder "19-install-mariadb" -Data @{
    ...
    if ($hasCustomPath) {
        $dbInstallDir = Join-Path $InstallPath "mariadb"
