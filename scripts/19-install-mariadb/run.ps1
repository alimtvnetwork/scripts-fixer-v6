...
. (Join-Path $scriptDir "helpers\mariadb.ps1")
...
if ($isSystem) {
    ...
    $installPath = Join-Path $devDir "mariadb"
}
...
# -- Install -------------------------------------------------------------------
$ok = Install-Mariadb -DbConfig $config.database -LogMessages $logMessages -InstallPath $installPath
