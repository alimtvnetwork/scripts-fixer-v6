...
function Install-Duckdb {
    <#
    .SYNOPSIS
        Installs DuckDB and verifies the installation.
    ...
    if ($cmd) {
        ...
        Save-ResolvedData -ScriptFolder "28-install-duckdb" -Data @{
    ...
    if ($hasCustomPath) {
        $dbInstallDir = Join-Path $InstallPath "duckdb"
