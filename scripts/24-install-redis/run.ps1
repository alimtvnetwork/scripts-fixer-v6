...
. (Join-Path $scriptDir "helpers\redis.ps1")
...
if ($isSystem) {
    ...
    $installPath = Join-Path $devDir "redis"
}
...
# -- Install -------------------------------------------------------------------
$ok = Install-Redis -DbConfig $config.database -LogMessages $logMessages -InstallPath $installPath
