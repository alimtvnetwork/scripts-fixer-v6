...
. (Join-Path $scriptDir "helpers\litedb.ps1")
...
if ($isSystem) {
    ...
    $installPath = Join-Path $devDir "litedb"
}
...
# -- Install -------------------------------------------------------------------
$ok = Install-Litedb -DbConfig $config.database -LogMessages $logMessages -InstallPath $installPath
