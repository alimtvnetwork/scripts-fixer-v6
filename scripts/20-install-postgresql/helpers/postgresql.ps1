...
function Install-Postgresql {
    <#
    .SYNOPSIS
        Installs PostgreSQL and verifies the installation.
    ...
    if ($cmd) {
        ...
        Save-ResolvedData -ScriptFolder "20-install-postgresql" -Data @{
    ...
    if ($hasCustomPath) {
        $dbInstallDir = Join-Path $InstallPath "postgresql"
