...
if ($isSystem) {
    ...
    $installPath = Join-Path $devDir "postgresql"
}
...
# -- Install -------------------------------------------------------------------
$ok = Install-Postgresql -DbConfig $config.database -LogMessages $logMessages -InstallPath $installPath
