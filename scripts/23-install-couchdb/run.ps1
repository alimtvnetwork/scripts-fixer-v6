...
. (Join-Path $scriptDir "helpers\couchdb.ps1")
...
if ($isSystem) {
    ...
    $installPath = Join-Path $devDir "couchdb"
}
...
# -- Install -------------------------------------------------------------------
$ok = Install-Couchdb -DbConfig $config.database -LogMessages $logMessages -InstallPath $installPath
