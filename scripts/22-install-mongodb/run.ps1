...
. (Join-Path $scriptDir "helpers\mongodb.ps1")
...
if ($isSystem) {
    ...
    $installPath = Join-Path $devDir "mongodb"
}
...
# -- Install -------------------------------------------------------------------
$ok = Install-Mongodb -DbConfig $config.database -LogMessages $logMessages -InstallPath $installPath
