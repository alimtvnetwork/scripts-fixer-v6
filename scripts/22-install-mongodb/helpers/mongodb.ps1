...
function Install-Mongodb {
    <#
    .SYNOPSIS
        Installs MongoDB and verifies the installation.
    ...
    if ($cmd) {
        ...
        Save-ResolvedData -ScriptFolder "22-install-mongodb" -Data @{
    ...
    if ($hasCustomPath) {
        $dbInstallDir = Join-Path $InstallPath "mongodb"
